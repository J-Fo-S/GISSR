...
def basis_exchange(W, H, k_range, segment_width, seq_nmf = True):
    ...
    class_index = nmfsc_clustering(period_feature, k_range, seq_nmf)
...
def nmfsc_clustering(data, k_range, sH=0.3, iter_num = 500, replica = 10, seq_nmf = True):
    ...
    if seq_nmf == True:
        W, H, cost, loading, power = seqnmf(data, K=len(k_range), L=4, Lambda=0.001)
